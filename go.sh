...
function getdocker {
	...
	sudo apt update && sudo apt install -y docker-ce docker-ce-cli containerd.io docker-compose-plugin && sudo groupadd docker && sudo usermod -aG docker $USER && sudo -u $USER -g docker -p "bash go.sh"
	exit 2
}
