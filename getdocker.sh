...
sudo apt update && sudo apt install -y docker-ce docker-ce-cli containerd.io docker-compose-plugin && sudo groupadd docker && sudo usermod -aG docker $USER && sudo su $USER --session-command "bash go.sh"
exit 2